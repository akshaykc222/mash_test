...
abstract class VehicleTrackerDataSource{
  Future<List<VehicleTrackerEntity>>  getVehicleTrackerStop(VehicleTrackerRequest request);
...
class VehicleTrackerImpl extends VehicleTrackerDataSource{
  ...
  @override
  Future<List<VehicleTrackerEntity>> getVehicleTrackerStop(VehicleTrackerRequest request) async {
    final data = await  apiProvider.get(AppRemoteRoutes.vehicleTrackerStops,body: request.toJson());
    return List<VehicleTrackerEntity>.from(data["resTable"].map((e)=>VehicleTrackerModel.fromJson(e))).toList();
  }
